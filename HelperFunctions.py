...
def read_virus_file(file):
    ...
    with open(file, "r") as virus:
        ...
        data["general_transmission"] = float(virus[2][8 : len(virus[2]) - 1])  / 100
        data["mask_transmission"] = float(virus[3][10 : len(virus[3]) - 1])  / 100
        data["distance_transmission"] = float(virus[4][17: len(virus[4]) - 1])  / 100
